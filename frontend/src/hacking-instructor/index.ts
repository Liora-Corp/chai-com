...
function loadHint(hint: ChallengeHint): HTMLElement {
  ...
  const picture = createElement("img", pictureStyles, {
  ...
  const cancelButton = createElement("button", cancelButtonStyles, {
    id: "cancelButton",
    title: "Cancel the tutorial",
  });
  ...
  const relAnchor = createElement("div", {
    position: "relative",
    display: "inline",
  });
...
export async function startHackingInstructorFor(
  ...
  for (const hint of challengeInstruction.hints) {
    ...
    continueConditions.push(
      waitForCancel(document.getElementById("cancelButton")),
    );
