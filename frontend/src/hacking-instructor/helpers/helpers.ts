...
export function waitForInputToHaveValue(
  inputSelector: string,
  value: string,
  options: any = { ignoreCase: true, replacement: [] },
) {
  return async () => {
    ...
    while (true) {
      if (
        options.ignoreCase &&
        inputElement.value.toLowerCase() === value.toLowerCase()
      ) {
        break;
...
export function waitForElementsInnerHtmlToBe(
...
export function waitForSelectToHaveValue(
...
export function waitForSelectToNotHaveValue(
  selectSelector: string,
  value: string,
) {
  return async () => {
...
export function waitForRightUriQueryParamPair(key: string, value: string) {
  return async () => {
    while (true) {
      const encodedValue: string = encodeURIComponent(value).replace(
        /%3A/g,
        ":",
      );
