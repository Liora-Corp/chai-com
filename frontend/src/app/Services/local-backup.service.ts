...
export class LocalBackupService {
  ...
  save(fileName = "owasp_chai_com") {
    ...
    forkJoin([continueCode, continueCodeFindIt, continueCodeFixIt]).subscribe({
      ...
      error: () => {
        console.log(
          "Failed to retrieve continue code(s) for backup from server. Using cookie values as fallback.",
        );
        backup.continueCode = this.cookieService.get("continueCode")
          ? this.cookieService.get("continueCode")
          : undefined;
        backup.continueCodeFindIt = this.cookieService.get("continueCodeFindIt")
          ? this.cookieService.get("continueCodeFindIt")
          : undefined;
        backup.continueCodeFixIt = this.cookieService.get("continueCodeFixIt")
  ...
  restore(backupFile: File) {
    return from(
      backupFile
        ...
        .then((backupData) => {
          ...
          if (backup.version === this.VERSION) {
            this.restoreCookie(
              ...
              backup.banners?.welcomeBannerStatus,
            );
            this.restoreCookie(
              "cookieconsent_status",
              backup.banners?.cookieConsentStatus,
            );
            ...
            const snackBarRef = this.snackBar.open(
              "Backup has been restored from " + backupFile.name,
              "Apply changes now",
              {
                duration: 10000,
                panelClass: ["mat-body"],
            ...
            snackBarRef.onAction().subscribe(() => {
              const hackingProgress = backup.continueCode
                ? this.challengeService.restoreProgress(
                    encodeURIComponent(backup.continueCode),
                  )
                : of(true);
              const findItProgress = backup.continueCodeFindIt
                ? this.challengeService.restoreProgressFindIt(
                    encodeURIComponent(backup.continueCodeFindIt),
                  )
                : of(true);
              const fixItProgress = backup.continueCodeFixIt
                ? this.challengeService.restoreProgressFixIt(
                    encodeURIComponent(backup.continueCodeFixIt),
                  )
                : of(true);
              forkJoin([
                hackingProgress,
            ...
            this.snackBarHelperService.open(
              `Version ${backup.version} is incompatible with expected version ${this.VERSION}`,
              "errorBar",
            );
          }
        })
        .catch((err: Error) => {
          this.snackBarHelperService.open(
            `Backup restore operation failed: ${err.message}`,
            "errorBar",
          );
        }),
    );
