...
describe("DataSubjectService", () => {
  ...
  it("should pass the erasure request directly to the rest API", inject(
    [DataSubjectService, HttpTestingController],
    fakeAsync(
      (service: DataSubjectService, httpMock: HttpTestingController) => {
        let res: any;
        service.erase({}).subscribe((data) => (res = data));
        const req = httpMock.expectOne(
          "http://localhost:3000/rest/user/erasure-request",
        );
  ...
  it("should request data export directly from the rest api", inject(
    [DataSubjectService, HttpTestingController],
    fakeAsync(
      (service: DataSubjectService, httpMock: HttpTestingController) => {
        let res: any;
