...
describe("ProductService", () => {
  ...
  it("should search for products directly from the rest api", inject(
  ...
  it("should get all products directly from the rest api", inject(
    [ProductService, HttpTestingController],
    fakeAsync((service: ProductService, httpMock: HttpTestingController) => {
      let res: any;
  ...
  it("should get single product directly from the rest api", inject(
    [ProductService, HttpTestingController],
    fakeAsync((service: ProductService, httpMock: HttpTestingController) => {
      let res: any;
      service.get(1).subscribe((data) => (res = data));
      const req = httpMock.expectOne(
        "http://localhost:3000/api/Products/1?d=" +
          encodeURIComponent(new Date().toDateString()),
      );
      req.flush({ data: "apiResponse" });
