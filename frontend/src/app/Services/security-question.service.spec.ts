...
describe("SecurityQuestionService", () => {
  ...
  it("should get all challenges directly from the rest api", inject(
    [SecurityQuestionService, HttpTestingController],
    fakeAsync(
      (service: SecurityQuestionService, httpMock: HttpTestingController) => {
        let res: any;
  ...
  it("should get security question by user email directly from the rest api", inject(
    [SecurityQuestionService, HttpTestingController],
    fakeAsync(
      (service: SecurityQuestionService, httpMock: HttpTestingController) => {
        let res: any;
        ...
        const req = httpMock.expectOne(
          "http://localhost:3000/rest/user/security-question?email=x@y.z",
        );
