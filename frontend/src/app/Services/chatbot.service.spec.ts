...
describe("ChatbotService", () => {
  ...
  it("should get status from the REST API", inject(
    [ChatbotService, HttpTestingController],
    fakeAsync((service: ChatbotService, httpMock: HttpTestingController) => {
      let res: any;
      service.getChatbotStatus().subscribe((data) => (res = data));
      const req = httpMock.expectOne(
        "http://localhost:3000/rest/chatbot/status",
      );
  ...
  it("should get query response from the REST API", inject(
    [ChatbotService, HttpTestingController],
    fakeAsync((service: ChatbotService, httpMock: HttpTestingController) => {
      let res: any;
      service
        .getResponse("query", "apiQuery")
        .subscribe((data) => (res = data));
      const req = httpMock.expectOne(
        "http://localhost:3000/rest/chatbot/respond",
      );
