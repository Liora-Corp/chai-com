...
describe("SearchResultComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    basketService = jasmine.createSpyObj("BasketService", [
      "find",
      "get",
      "put",
      "save",
  ...
  it("should render product descriptions as trusted HTML", () => {
    productService.search.and.returnValue(
      of([{ description: '<script>alert("XSS")</script>' }]),
    );
    component.ngAfterViewInit();
    fixture.detectChanges();
  ...
  it("should notify socket if search query includes DOM XSS payload while filtering table", () => {
    activatedRoute.setQueryParameter(
      '<iframe src="javascript:alert(`xss`)"> Payload',
    );
    spyOn(mockSocket, "emit");
    component.filterTable();
    expect(mockSocket.emit.calls.mostRecent().args[0]).toBe(
      "verifyLocalXssChallenge",
    );
    expect(mockSocket.emit.calls.mostRecent().args[1]).toBe(
      activatedRoute.snapshot.queryParams.q,
    );
  });
  ...
  it("should add new product to basket", () => {
    ...
    expect(translateService.get).toHaveBeenCalledWith("BASKET_ADD_PRODUCT", {
  ...
  it("should translate BASKET_ADD_PRODUCT message", () => {
    ...
    translateService.get.and.returnValue(
      of("Translation of BASKET_ADD_PRODUCT"),
    );
    sessionStorage.setItem("bid", "4711");
    component.addToBasket(1);
    expect(basketService.find).toHaveBeenCalled();
  ...
  it("should add similar product to basket", () => {
    basketService.find.and.returnValue(
      of({
        Products: [
          { id: 1 },
          { id: 2, name: "Tomato Juice", BasketItem: { id: 42 } },
        ],
      }),
    );
  ...
  it("should translate BASKET_ADD_SAME_PRODUCT message", () => {
    ...
    productService.get.and.returnValue(of({ name: "Tomato Juice" }));
    translateService.get.and.returnValue(
      of("Translation of BASKET_ADD_SAME_PRODUCT"),
    );
    sessionStorage.setItem("bid", "4711");
    component.addToBasket(2);
    expect(basketService.find).toHaveBeenCalled();
