...
export class ContactComponent implements OnInit {
  ...
  public authorControl: UntypedFormControl = new UntypedFormControl(
    { value: "", disabled: true },
    [],
  );
  public feedbackControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
    ...
    this.userService.whoAmI().subscribe({
      next: (data: any) => {
        ...
        this.authorControl.setValue(
          data.email ? `***${data.email.slice(3)}` : "anonymous",
        );
      },
      error: (err) => {
