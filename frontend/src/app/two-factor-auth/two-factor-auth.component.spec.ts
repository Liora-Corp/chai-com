...
describe("TwoFactorAuthComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    twoFactorAuthService = jasmine.createSpyObj("TwoFactorAuthService", [
      "status",
      "setup",
      "disable",
    ]);
  ...
  it("should set TOTP secret and URL if 2FA is not already set up", () => {
    ...
    expect(component.totpUrl).toBe(
      "otpauth://totp/Test%20App:email?secret=secret&issuer=Test%20App",
    );
  ...
  it("should confirm successful setup of 2FA", () => {
    ...
    expect(component.setupStatus).toBe(true);
    expect(twoFactorAuthService.setup).toHaveBeenCalledWith(
      "password",
      "12345",
      undefined,
    );
  ...
  it("should reset and mark form as errored when 2FA setup fails", () => {
    ...
    expect(component.twoFactorSetupForm.get("passwordControl").pristine).toBe(
      false,
    );
    expect(
      component.twoFactorSetupForm.get("initialTokenControl").pristine,
    ).toBe(false);
    component.setup();
  ...
  it("should reset and mark form as errored when disabling 2FA fails", () => {
    twoFactorAuthService.disable.and.returnValue(
    ...
    expect(component.twoFactorDisableForm.get("passwordControl").pristine).toBe(
      false,
    );
    component.disable();
