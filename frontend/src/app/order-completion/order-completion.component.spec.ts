...
describe("OrderCompletionComponent", () => {
  ...
  it("should hold order details returned by backend API", () => {
    trackOrderService.find.and.returnValue(
      of({
        data: [
          {
            totalPrice: 2.88,
    ...
    expect(component.orderDetails.products[0].name).toBe(
      "Apple Juice (1000ml)",
    );
  ...
  it("should have bullet point list of products in tweet", () => {
    trackOrderService.find.and.returnValue(
      of({ data: [{ products: [{ name: "A" }, { name: "B" }] }] }),
    );
    configurationService.getApplicationConfiguration.and.returnValue(of({}));
    component.ngOnInit();
  ...
  it("should truncate tweet text if it exceeds 140 characters", () => {
    ...
    configurationService.getApplicationConfiguration.and.returnValue(of({}));
    component.ngOnInit();
    expect(component.tweetText).toBe(
      "I just purchased%0a- AAAAAAAAAAAAAAAAAAAA%0a- BBBBBBBBBBBBBBBBBBBB%0a- CCCCCCCCCCCCCCCCCCCC%0a- DDDDDDDDDDDDDDDDDDDD%0a- EEEEEEEEEEEEEEEEEEE...",
    );
  });

  it("should derive twitter handle from twitter URL if configured", () => {
    trackOrderService.find.and.returnValue(of({ data: [{ products: [] }] }));
    configurationService.getApplicationConfiguration.and.returnValue(
      of({
        application: {
          social: { twitterUrl: "https://twitter.com/bkimminich" },
        },
      }),
    );
    component.ngOnInit();
  ...
  it("should append twitter handle to truncated tweet text", () => {
    ...
    configurationService.getApplicationConfiguration.and.returnValue(
      of({
        application: {
          social: { twitterUrl: "https://twitter.com/owasp_chaicom" },
        },
      }),
    );
    component.ngOnInit();
    expect(component.tweetText).toBe(
      "I just purchased%0a- AAAAAAAAAAAAAAAAAAAA%0a- BBBBBBBBBBBBBBBBBBBB%0a- CCCCCCCCCCCCCCCCCCCC%0a- DDDDDDDDDDDDDDDDDDDD%0a- EEEEEEEEEEEEEEEEEEE...%0afrom @owasp_chaicom",
    );
  });
  ...
  it("should use configured URL as is if it is not a twitter URL", () => {
    trackOrderService.find.and.returnValue(of({ data: [{ products: [] }] }));
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ application: { social: { twitterUrl: "http://localhorst:42" } } }),
    );
    component.ngOnInit();
    expect(component.tweetText).toBe(
      "I just purchased%0afrom http://localhorst:42",
    );
  });
  ...
  it("should use configured application name as a fallback for missing twitter URL", () => {
    ...
    configurationService.getApplicationConfiguration.and.returnValue(
      of({
        application: { name: "WowChai-Com", social: { twitterUrl: null } },
      }),
    );
    component.ngOnInit();
    expect(component.tweetText).toBe("I just purchased%0afrom WowChai-Com");
  });
