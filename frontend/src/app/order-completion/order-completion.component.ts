    ...
    this.activatedRoute.paramMap.subscribe({
      next: (paramMap: ParamMap) => {
        ...
        this.trackOrderService.find(this.orderId).subscribe({
          next: (results) => {
            this.promotionalDiscount = results.data[0].promotionalAmount
              ? parseFloat(results.data[0].promotionalAmount)
              : 0;
            this.deliveryPrice = results.data[0].deliveryPrice
              ? parseFloat(results.data[0].deliveryPrice)
              : 0;
              ...
              next: (config) => {
                if (config?.application?.social) {
                  ...
                  if (config.application.social.twitterUrl) {
                    this.tweetText +=
                      config.application.social.twitterUrl.replace(
                        "https://twitter.com/",
                        "@",
                      );
                  } else {
