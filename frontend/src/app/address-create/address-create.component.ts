...
export class AddressCreateComponent implements OnInit {
  ...
  public countryControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
  ]);
  public nameControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
  ]);
  public numberControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
    Validators.min(1111111),
    Validators.max(9999999999),
  ]);
  public pinControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
    ...
    if (this.mode === "edit") {
      this.addressService.put(this.addressId, this.address).subscribe({
        next: (savedAddress) => {
          ...
          this.translate
            .get("ADDRESS_UPDATED", { city: savedAddress.city })
            .subscribe({
              next: (addressUpdated) => {
      ...
      this.addressService.save(this.address).subscribe({
        next: (savedAddress) => {
          ...
          this.routeToPreviousUrl();
          this.translate
            .get("ADDRESS_ADDED", { city: savedAddress.city })
            .subscribe({
              next: (addressAdded) => {
