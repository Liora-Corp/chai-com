...
describe("LoginGuard", () => {
  ...
  it("should open for authenticated users", inject(
    [LoginGuard],
    (guard: LoginGuard) => {
  ...
  it("should close for anonymous users", inject(
    [LoginGuard],
    (guard: LoginGuard) => {
  ...
  it("returns payload from decoding a valid JWT", inject(
    [LoginGuard],
    (guard: LoginGuard) => {
  ...
  it("returns nothing when decoding an invalid JWT", inject(
    [LoginGuard],
    (guard: LoginGuard) => {
  ...
  it("returns nothing when decoding an non-existing JWT", inject(
    [LoginGuard],
    (guard: LoginGuard) => {
      localStorage.removeItem("token");
      expect(guard.tokenDecode()).toBeNull();
...
describe("AdminGuard", () => {
  ...
  beforeEach(() => {
    loginGuard = jasmine.createSpyObj("LoginGuard", [
      "tokenDecode",
      "forbidRoute",
    ]);
  ...
  it("should close for regular customers", inject(
    [AdminGuard],
    (guard: AdminGuard) => {
  ...
  it("should close for deluxe customers", inject(
    [AdminGuard],
    (guard: AdminGuard) => {
  ...
  it("should close for accountants", inject(
    [AdminGuard],
    (guard: AdminGuard) => {
...
describe("AccountingGuard", () => {
  ...
  it("should open for accountants", inject(
    [AccountingGuard],
    (guard: AccountingGuard) => {
      loginGuard.tokenDecode.and.returnValue({ data: { role: "accounting" } });
      expect(guard.canActivate()).toBeTrue();
  ...
  it("should close for admins", inject(
    [AccountingGuard],
    (guard: AccountingGuard) => {
      loginGuard.tokenDecode.and.returnValue({ data: { role: "admin" } });
      expect(guard.canActivate()).toBeFalse();
...
describe("DeluxeGuard", () => {
  ...
  it("should open for deluxe customers", inject(
    [DeluxeGuard],
    (guard: DeluxeGuard) => {
      loginGuard.tokenDecode.and.returnValue({ data: { role: "deluxe" } });
