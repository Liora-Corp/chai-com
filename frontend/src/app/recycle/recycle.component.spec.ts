...
describe("RecycleComponent", () => {
  ...
  it("should display pickup message and reset recycle form on saving when pickup is selected", () => {
    recycleService.save.and.returnValue(
      of({ isPickup: true, pickupDate: "10/7/2018" }),
    );
    userService.whoAmI.and.returnValue(of({}));
    ...
    expect(translateService.get).toHaveBeenCalledWith(
      "CONFIRM_RECYCLING_PICKUP",
      { pickupdate: "10/7/2018" },
    );
  });
  ...
  it("should use a configured product image on top of page", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ application: { recyclePage: { topProductImage: "top.png" } } }),
    );
    component.ngOnInit();
  ...
  it("should use a configured product image on bottom of page", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ application: { recyclePage: { topProductImage: "bottom.png" } } }),
    );
    component.ngOnInit();
    expect(component.topImage).toEqual(
      "assets/public/images/products/bottom.png",
    );
  });
