...
describe("ChallengeSolvedNotificationComponent", () => {
  ...
  it("should add new notification", fakeAsync(() => {
    ...
    expect(translateService.get).toHaveBeenCalledWith("CHALLENGE_SOLVED", {
      challenge: "Test",
    });
    expect(component.notifications).toEqual([
      {
        key: "test",
        message: "CHALLENGE_SOLVED",
        flag: "1234",
        copied: false,
        country: undefined,
      },
  ...
  it("should show CTF flag codes if configured accordingly", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ ctf: { showFlagsInNotifications: true } }),
    );
    component.ngOnInit();
  ...
  it("should hide CTF flag codes if configured accordingly", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ ctf: { showFlagsInNotifications: false } }),
    );
    component.ngOnInit();
  ...
  it("should not load countries for FBCTF when configured to hide country details", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ ctf: { showCountryDetailsInNotifications: "none" } }),
    );
    component.ngOnInit();
