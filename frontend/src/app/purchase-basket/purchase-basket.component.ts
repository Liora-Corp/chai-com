    ...
    this.basketService
      ...
      .subscribe({
        next: (basket) => {
          ...
          this.itemTotal = basket.Products.reduce(
            (itemTotal, product) =>
              itemTotal + product.price * product.BasketItem.quantity,
            0,
          );

          this.bonus = basket.Products.reduce(
            (bonusPoints, product) =>
              bonusPoints +
              Math.round(product.price / 10) * product.BasketItem.quantity,
            0,
          );
  ...
  addToQuantity(id, value) {
    this.basketService.get(id).subscribe({
      next: (basketItem) => {
        ...
        this.basketService
          .put(id, { quantity: newQuantity < 1 ? 1 : newQuantity })
          .subscribe({
            next: () => {
              this.load();
              this.basketService.updateNumberOfCartItems();
