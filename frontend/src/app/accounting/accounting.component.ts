...
export class AccountingComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild("paginatorOrderHistory", { static: true })
  paginatorOrderHistory: MatPaginator;
  ...
  modifyQuantity(id, value) {
    this.quantityService
      .put(id, { quantity: value < 0 ? 0 : value })
      .subscribe({
        next: (quantity) => {
          ...
          this.snackBarHelperService.open(
            `Quantity for ${product.name} has been updated.`,
            "confirmBar",
          );
          this.loadQuantity();
        },
  ...
  modifyPrice(id, value) {
    this.productService.put(id, { price: value < 0 ? 0 : value }).subscribe({
      next: (product) => {
        this.snackBarHelperService.open(
          `Price for ${product.name} has been updated.`,
          "confirmBar",
        );
        this.loadProducts();
      },
  ...
  changeDeliveryStatus(deliveryStatus, orderId) {
    this.orderHistoryService
      .toggleDeliveryStatus(orderId, { deliveryStatus })
      .subscribe({
        next: () => {
          this.loadOrders();
        },
