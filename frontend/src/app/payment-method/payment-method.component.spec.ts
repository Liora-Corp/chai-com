...
describe("PaymentMethodComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    paymentService = jasmine.createSpyObj("BasketService", [
      "save",
      "get",
      "del",
    ]);
  ...
  it("should hold cards returned by backend API", () => {
    paymentService.get.and.returnValue(
      of([{ cardNum: "************1231" }, { cardNum: "************6454" }]),
    );
    component.load();
  ...
  it("should reset the form on saving card and show confirmation", () => {
    ...
    component.save();
    expect(translateService.get).toHaveBeenCalledWith("CREDIT_CARD_SAVED", {
      cardnumber: "1234",
    });
