...
describe("AdministrationComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    userService.find.and.returnValue(
      of([{ email: "User1" }, { email: "User2" }]),
    );
    feedbackService = jasmine.createSpyObj("FeedbackService", ["find", "del"]);
    feedbackService.find.and.returnValue(
      of([{ comment: "Feedback1" }, { comment: "Feedback2" }]),
    );
  ...
  it("should open the FeedbackDetailsComponent to show details", () => {
    ...
    expect(dialog.open).toHaveBeenCalledWith(FeedbackDetailsComponent, {
      data: { feedback: "Feedback", id: 1 },
    });
  });
