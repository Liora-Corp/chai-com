...
describe("DataExportComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    imageCaptchaService = jasmine.createSpyObj("ImageCaptchaService", [
      "getCaptcha",
    ]);
    imageCaptchaService.getCaptcha.and.returnValue(of({}));
    dataSubjectService = jasmine.createSpyObj("DataSubjectService", [
      "dataExport",
    ]);
  ...
  it("should store the captcha on getting new captcha", () => {
    imageCaptchaService.getCaptcha.and.returnValue(
      of({ image: "<svg>captcha</svg>" }),
    );
    component.getNewCaptcha();
    const sanitezedCaptcha = domSanitizer.sanitize(
  ...
  it("should show the confirmation and fetch user data and reset data export form on requesting data export", () => {
    dataSubjectService.dataExport.and.returnValue(
      of({ confirmation: "Data being exported", userData: "{ user data }" }),
    );
