...
describe("PaymentComponent", () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    userService.isLoggedIn = jasmine.createSpyObj("userService.isLoggedIn", [
      "next",
    ]);
      ...
      imports: [
        RouterModule.forRoot([
          ...
          { path: "wallet", component: WalletComponent },
          {
            ...
            component: DummyDeluxeMembershipComponent,
          },
        ]),
  ...
  it("should use custom reddit URL if configured", () => {
    configurationService.getApplicationConfiguration.and.returnValue(
      of({ application: { social: { redditUrl: "reddit" } } }),
    );
    component.ngOnInit();
  ...
  it("should accept a valid coupon code", () => {
    ...
    expect(translateService.get).toHaveBeenCalledWith("DISCOUNT_APPLIED", {
      discount: 42,
    });
  ...
  it("should translate DISCOUNT_APPLIED message", () => {
    ...
    expect(component.couponConfirmation).toBe(
      "Translation of DISCOUNT_APPLIED",
    );
    expect(component.couponError).toBeUndefined();
  });
