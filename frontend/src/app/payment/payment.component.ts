...
export class PaymentComponent implements OnInit {
  ...
  public applicationName = "WowChai-Com";
  private campaignCoupon: string;
  public couponControl: UntypedFormControl = new UntypedFormControl("", [
    Validators.required,
  ...
  ngOnInit(): void {
    ...
    this.couponPanelExpanded = localStorage.getItem("couponPanelExpanded")
      ? JSON.parse(localStorage.getItem("couponPanelExpanded"))
      : false;
    this.paymentPanelExpanded = localStorage.getItem("paymentPanelExpanded")
    ...
    this.activatedRoute.paramMap.subscribe({
      next: (paramMap: ParamMap) => {
        ...
        if (this.mode === "wallet") {
          ...
          const promotionalDiscount = sessionStorage.getItem("couponDiscount")
  ...
  applyCoupon() {
    ...
    const offsetTimeZone =
      (this.clientDate.getTimezoneOffset() + 60) * 60 * 1000;
  ...
  choosePayment() {
    ...
    if (this.mode === "wallet") {
      this.walletService
        ...
        .subscribe({
          next: () => {
            ...
            this.ngZone.run(
              async () => await this.router.navigate(["/wallet"]),
            );
      ...
      this.userService
        ...
        .subscribe({
          next: (data) => {
            ...
            this.ngZone.run(
              async () => await this.router.navigate(["/deluxe-membership"]),
            );
          },
          error: (err) => {
            console.log(err);
      ...
      this.ngZone.run(
        async () => await this.router.navigate(["/order-summary"]),
      );
    }
  }
